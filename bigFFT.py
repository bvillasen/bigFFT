...
rho[sphere] = 1
rho = r
data = np.zeros_like( rho, dtype=np.complex )
data.real = rho.copy()
...
stride = 1
outFile.create_dataset('density', data=data[::stride,::stride,::stride] )


slab_data_gather = np.zeros( [slab_nProcs, slab_nz, slab_ny_local, slab_nx_local], dtype=np.complex )
...
slab_data = np.zeros( [slab_nz, slab_ny, slab_nx ], dtype=np.complex )
for i,pId_temp in enumerate(slab_procs_ids):
...
# Make 2D FFTs for XY planes
if pId == 0: print '\nComputing 2D FFTs...\n'
fft_2d_data = np.zeros( [slab_nz, slab_ny, slab_nx ], dtype=np.complex )
for k in range(slab_nz):
  fft_2d_data[k,:,:] = fft2( slab_data[k,:,:] ).copy()
MPIcomm.Barrier()

outFile.create_dataset('slab_gather', data=slab_data_gather[::stride,::stride,::stride] )
outFile.create_dataset('slab', data=slab_data[::stride,::stride,::stride] )

MPIcomm.Barrier()
# slab_send = slab_data.copy()
slab_send = fft_2d_data.copy()

slab2_nx = nx_total
slab2_ny = ny_total / nProcess
slab2_nz = nz_total
slab2_nz_gather = nz_total/nProcess
slab2_gather = np.zeros([nProcess, slab2_nz_gather, slab2_ny, slab2_nx], dtype=np.complex )
...
slab2_data = np.zeros( [slab2_nz, slab2_ny, slab2_nx ], dtype=np.complex )
for pId_temp in range(nProcess):
...
# Make 1D FFTs for Z axes
if pId == 0: print '\nComputing 1D FFTs...\n'
fft_3d_data = np.zeros_like( slab2_data )
ny, nx = ny_total/nProcess, nx_total
for i in range(ny):
  for j in range(nx):
    fft_3d_data[:,i,j] = fft( slab2_data[:,i,j] ).copy()
MPIcomm.Barrier()

outFile.create_dataset('slab2_gather', data=slab2_gather[::stride,::stride,::stride])
outFile.create_dataset('slab2', data=slab2_data[::stride,::stride,::stride])

# slab2_send = slab2_data.copy()
slab2_send = fft_3d_data.copy()

slab3_gather = np.zeros([ nProcess, slab_nz, slab2_ny, slab_nx ], dtype=np.complex )
for pId_temp in range(nProcess):
  slab2_domain_z_start, slab2_domain_z_end = pId_temp*slab2_nz_gather, (pId_temp+1)*slab2_nz_gather
...
slab3_data = np.zeros( [slab_nz, slab_ny, slab_nx ], dtype=np.complex )
for pId_temp in range(nProcess):
  slab2_domain_y_start, slab2_domain_y_end = pId_temp*slab2_ny, (pId_temp+1)*slab2_ny
  slab3_data[:,slab2_domain_y_start:slab2_domain_y_end,:] = slab3_gather[pId_temp].copy()

outFile.create_dataset('slab3_gather', data=slab3_gather[::stride,::stride,::stride] )
outFile.create_dataset('slab3', data=slab_data[::stride,::stride,::stride] )

slab3_send = slab3_data.copy()
slab4_gather = np.zeros( [slab_nProcs, slab_nz, slab_ny_local, slab_nx_local], dtype=np.complex )
for i,pId_temp in enumerate(slab_procs_ids):
  pId_z_temp, pId_y_temp, pId_x_temp = get_mpi_id_3D( pId_temp, nP_x, nP_y )
  slab_domain_x_start, slab_domain_x_end = pId_x_temp*nx_local, (pId_x_temp+1)*nx_local
  slab_domain_y_start, slab_domain_y_end = pId_y_temp*ny_local, (pId_y_temp+1)*ny_local
...
slab4_data = np.zeros( [nz_local, ny_local, nx_local], dtype=np.complex )
for i,pId_temp in enumerate(slab_procs_ids):
  slab_domain_z_start, slab_domain_z_end = i*slab_nz, (i+1)*slab_nz
  slab4_data[slab_domain_z_start:slab_domain_z_end,:,:] = slab4_gather[i]

outFile.create_dataset('slab4_gather', data=slab4_gather[::stride,::stride,::stride] )
outFile.create_dataset('slab4', data=slab4_data[::stride,::stride,::stride] )
# outFile.create_dataset('fft', data=slab4_data[::stride,::stride,::stride] )

# diff =  np.abs(slab4_data - rho)
# msg = 'Diff: {0} , {1}'.format( diff.max(), diff.min())
# print_mpi( msg , pId, nProcess, MPIcomm )
# outFile.close()
